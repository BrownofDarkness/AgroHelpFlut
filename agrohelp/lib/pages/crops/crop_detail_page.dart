...
class CropDetailPage extends StatefulWidget {
  ...
  const CropDetailPage(
      {Key? key, required this.index, required this.id, required this.side})
      : super(key: key);
...
class _CropDetailPageState extends State<CropDetailPage> {
  ...
  Future<void> _loadressource(int id) async {
    paletteGenerator = await PaletteGenerator.fromImageProvider(NetworkImage(
        Get.find<CultureController>()
            .suggestedcultureList[widget.index]['culture']
            .photo));
    setState(() {
      mainColor = paletteGenerator.lightMutedColor?.color != null
          ? paletteGenerator.lightMutedColor?.color
          : Color.fromARGB(255, 220, 238, 213);
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: GetBuilder<CultureController>(builder: (cultures) {
          Map culture = widget.side == "r"
              ? cultures.suggestedcultureList[widget.index]
              : cultures.popularcultureList[widget.index];
          return Stack(
            children: [
              //background Image
              Positioned(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      StarsItem(
                        fav: widget.side == "r"
                            ? culture["favorite"]
                            : cultures.checkIfPopisRec(culture["culture"].id),
                        rec: widget.side == "r"
                            ? true
                            : cultures.checkIfPopisRec(culture["culture"].id),
                        cult: culture["culture"].id,
                        detail: true,
                      ),
                    ],
                  )),
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.only(
                          topRight:
                              Radius.circular(Dimensions.radius30(context) * 2),
                          topLeft: Radius.circular(
                              Dimensions.radius30(context) * 2)),
                      color: mainColor,
                    ),
                    child: Column(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  children: [
                                    for (int i = 0;
                                        i <
                                            cultures.cultureDetails['practises']
                                                .length;
                                        i++) ...[
                                      Container(
                                        width: double.maxFinite,
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            Expanded(
                                              child: BigText(
                                                text:
                                                    "${cultures.cultureDetails['practises'][i]['name']} : ${cultures.cultureDetails['practises'][i]['practise']}",
                                                size:
                                                    Dimensions.font16(context) *
                                                        0.8,
                                              ),
                                            ),
                                          ],
                                ...
                                Container(
                                  ...
                                  child: PageView.builder(
                                      ...
                                      itemCount: cultures
                                          .cultureDetails['fertilizers'].length,
                                      itemBuilder: (context, position) {
                                        return _buildPageItem(
                                            position,
                                            cultures.cultureDetails[
                                                'fertilizers'][position]);
                                      }),
                                ),
                                SizedBox(
                                ...
                                Container(
                                  ...
                                  child: PageView.builder(
                                      ...
                                      itemCount: cultures
                                          .cultureDetails['diseases'].length,
                                      itemBuilder: (context, position) {
                                        return _buildPageItem2(
                                            position,
                                            cultures.cultureDetails['diseases']
                                                [position]);
                                      }),
                                ),
                                SizedBox(
                                ...
                                Container(
                                  ...
                                  child: PageView.builder(
                                      ...
                                      itemCount: cultures
                                          .cultureDetails['soils'].length,
                                      itemBuilder: (context, position) {
                                        return _buildPageItem3(
                                            position,
                                            cultures.cultureDetails['soils']
                                                [position]);
                                      }),
                                ),
                                SizedBox(
  ...
  Widget _buildPageItem2(
    ...
    if (index == _currpagevalue2.floor()) {
      ...
      currScale =
          _scaleFactor + (_currpagevalue2 - index + 1) * (1 - _scaleFactor2);
      currTrans = _height(context) * (1 - currScale) / 2;
    } else if (index == _currpagevalue2.floor() - 1) {
      currScale = 1 - (_currpagevalue2 - index) * (1 - _scaleFactor2);
      currTrans = _height(context) * (1 - currScale) / 2;
  ...
  Widget _buildPageItem3(
    ...
    if (index == _currpagevalue3.floor()) {
      ...
      currScale =
          _scaleFactor + (_currpagevalue3 - index + 1) * (1 - _scaleFactor3);
      currTrans = _height(context) * (1 - currScale) / 2;
    } else if (index == _currpagevalue3.floor() - 1) {
      currScale = 1 - (_currpagevalue3 - index) * (1 - _scaleFactor3);
      currTrans = _height(context) * (1 - currScale) / 2;
