...
class _SingUpPageState extends State<SingUpPage> {
  ...
  Widget build(BuildContext context) {
    var singUpImage = ["twitter.png", "facebook.png", "google.png"];
    ...
    void _registration(AuthController authController) {
      ...
      if (name.isEmpty) {
        ...
        ShowCustomSnackBar("the type is empty please choose one",
            title: "Type");
      } else if (type == "expert") {
        ShowCustomSnackBar(
            "the expert's account is not avalaible yet, please choose another one like 'agriculteur'.",
            title: "Type");
      } else {
        ShowCustomSnackBar("All  informations was typed",
            title: "Perfect", isError: false);
        SingUpModel signUpModel = SingUpModel(
            username: name, email: email, password: password, type: type);
        authController.registration(signUpModel).then((status) {
          if (status.isSucess == true) {
            ShowCustomSnackBar("user was successfully registered",
                title: "Registration", isError: false);
                ...
                Container(
                  ...
                  child: Center(
                    child: CircleAvatar(
                      ...
                      backgroundImage: AssetImage("assets/images/agrologo.png"),
                    ),
                  ),
                ),
                Text(
                  ...
                  decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          ...
                          blurRadius: 6,
                          offset:
                              Offset(0, 3), // décalage vers le bas et la droite
                        ),
                        BoxShadow(
                          color: Colors.white,
                          blurRadius: 5,
                          offset: Offset(
                              -2, -2), // décalage vers le haut et la gauche
                          spreadRadius: 0, // ne pas étendre l'ombre
                        ),
                      ]),
